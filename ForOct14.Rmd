...
x <- read.csv("/Users/jessicapevner/Downloads/Data_Extract_full.csv")
# Make the data long
xx <- reshape(x, 
...
```
Isolating World Entries
```{r}
# could also do for splits based on income or development status
worlddat <- xx[xx$Country.Name == "World",]
worlddat


temps <- read.csv("/Users/jessicapevner/Downloads/data.csv")
colnames(temps) <- c("year", "tempChange")
temps <- temps[5:nrow(temps),]
temps$year <- as.numeric(temps$year) + 1879 # workaround
